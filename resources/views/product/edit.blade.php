<x-product-layout>
    <div class="flex justify-between mb-[30px]">
        <h1 class="text-xl font-bold text-white ">Редактировать {{$product->name}}</h1>
    ...
    <form class="create-form" action="{{route('products.update', $product->id)}}" method="post">
        ...
        <div class="mb-[15px]">
            ...
            <div class="mt-2">
                <select name="status" id="status"
                        ...
                        class="block w-[470px] rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                    <option @if($product->status === 'available') selected @endif value="available">Доступен</option>
                    <option @if($product->status === 'unavailable') selected @endif value="unavailable">Не доступен
                    </option>
                </select>
            </div>
        </div>
        <div class="attributes mb-[30px]">
            ...
            <div class="attributes__content">
                @foreach($product->data as $name => $value)
                    <div class="attributes__item flex gap-x-[10px]">
                        <div class="mr-2">
                            <label for="attribute-name-0" class="block text-sm font-medium leading-6 text-white">Название</label>
                            <div class="mt-2">
                                <input type="text" value="{{$name}}" name="data[name][]" id="attribute-name-0"
                                       class="block w-[220px] rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                            </div>
                        </div>
                        <div class="">
                            <label for="attribute-value-0" class="block text-sm font-medium leading-6 text-white">Значение</label>
                            <div class="mt-2">
                                <input type="text" value="{{$value}}" name="data[value][]" id="attribute-value-0"
                                       class="block w-[220px] rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                            </div>
                        </div>
                        <div id="{{'attribute-delete-' . $loop->index}}"
                             class="attributes__delete self-end cursor-pointer relative z-50">
                            <svg width="10" height="11" class="relative z-0" viewBox="0 0 10 11" fill="none"
                                ...
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                      ...
                                      fill="#C4C4C4" fill-opacity="0.4"></path>
                            </svg>
        ...
        <button type="submit" class="bg-[#0FC5FF] py-[10px] text-center  w-[135px] rounded-md text-white">Сохранить
        </button>
    </form>
</x-product-layout>
