<x-product-layout>
    <div class="flex justify-between mb-[30px]">
        ...
        <div class="links flex items-center">
            <form class="h-[20px]" method="post" action="{{route('products.destroy', $product->id)}}">
                @csrf
                @method('DELETE')
                <button type="submit">
    ...
    <div class="product-info text-white">
        <div class="product-info__item flex gap-x-[40px]">
            <p class="w-[50px]">Артикул</p>
            <p>{{$product->article}}</p>
        </div>
        <div class="product-info__item flex gap-x-[40px]">
            <p class="w-[50px]"> Название</p>
            <p>{{$product->name}}</p>
        </div>
        <div class="product-info__item flex gap-x-[40px]">
            <p class="w-[50px]">Статус</p>
            <p>{{$product->status === 'available'? 'Доступен': 'Не доступен'}}</p>
        </div>
        <div class="product-info__item flex gap-x-[40px]">
            <p class="w-[50px]">Атрибуты</p>
            <div>
